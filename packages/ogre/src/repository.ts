...
export interface RespositoryObjectType {
    new<T extends { [k: string]: any }>(obj: T, options: RepositoryOptions<T>): RepositoryObject<T>;
}

/**
 * A repository recording and managing the state transitions of an object
 */
export class Repository<T extends { [k: PropertyKey]: any }> implements RepositoryObject<T> {
    constructor(
        obj: T,
    ...
    async commit(message: string, author: string, amend?: boolean): Promise<string> {
        ...
        if ((patch.length === 0 && !amend) || (amend && message === parent?.message)) {
            throw new Error(`no changes to commit`);
        }

        const timestamp = new Date();
        const parentChanges = amend && parent && (parent.changes?.length ?? 0 > 0) ? parent.changes : []
 ...
 * @param repository
 */
export const printChangeLog = <T extends { [k: string]: any }>(repository: RepositoryObject<T>) => {
    console.log("----------------------------------------------------------");
